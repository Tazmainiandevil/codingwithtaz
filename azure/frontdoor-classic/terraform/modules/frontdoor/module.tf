...
resource "azurerm_frontdoor" "instance" {
  ...
  dynamic "routing_rule" {
    ...
    content {
      ...
      frontend_endpoints = values({ for x, endpoint in var.frontend_endpoints : x => endpoint.name })
      dynamic "forwarding_configuration" {
        ...
        content {
          ...
          cache_query_parameter_strip_directive = forwarding_configuration.value.cache_enabled ? forwarding_configuration.value.cache_query_parameter_strip_directive : null
  ...
  dynamic "frontend_endpoint" {
    ...
    content {
      ...
      web_application_firewall_policy_link_id = frontend_endpoint.value.waf_policy_link_id == "" ? null : frontend_endpoint.value.waf_policy_link_id
    }
  }
}

resource "azurerm_frontdoor_custom_https_configuration" "custom_https" {
  for_each                          = { for x, endpoint in var.frontend_endpoints : x => endpoint }
  frontend_endpoint_id              = azurerm_frontdoor.instance.frontend_endpoints[each.value.name]
  custom_https_provisioning_enabled = each.value.custom_https_provisioning_enabled
  dynamic "custom_https_configuration" {
    for_each = each.value.custom_https_provisioning_enabled ? tolist(each.value.custom_https_configuration) : []
    content {
      certificate_source                         = custom_https_configuration.value.certificate_source
      azure_key_vault_certificate_vault_id       = custom_https_configuration.value.certificate_source == "AzureKeyVault" ? try(custom_https_configuration.value.certificate_vault_id, null) : null
      azure_key_vault_certificate_secret_name    = custom_https_configuration.value.certificate_source == "AzureKeyVault" ? try(custom_https_configuration.value.certificate_secret_name, null) : null
      azure_key_vault_certificate_secret_version = custom_https_configuration.value.certificate_source == "AzureKeyVault" ? try(custom_https_configuration.value.certificate_secret_version, null) : null
    }
  }
}
